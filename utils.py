...
from sklearn.model_selection import KFold, PredefinedSplit, StratifiedKFold, train_test_split
...
class MMPmodel:
    def __init__(self, model_name, X, Y, task_binary):
        self.X = X
        self.Y = Y
    ...
    def save_results(self, results, dataset_name, model, results_dir):
        """
...
class ModelTuner:
    def __init__(self, model, X, Y, task_binary, n_jobs=-1):
        self.model = model
        self.X = X
        self.Y = Y
        self.n_jobs = n_jobs
    ...
    def tune_para(self):
        mmp = MMPmodel(self.model, self.X, self.Y, self.task_binary)
        get_mdl = mmp.get_model(self.model)
        if self.model == 'SVM':
             param_grid = {
                'C': [0.1, 1, 10, 100],
